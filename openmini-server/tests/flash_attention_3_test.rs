...
use openmini_server::model::inference::flash_attention_3::{
    FlashAttention3, FlashAttention3Config,
};
...
fn test_flash_attention_3_config_creation() {
    ...
    println!(
        ...
        config.block_size, config.head_block_size
    );
}
...
fn test_flash_attention_3_forward_small() {
    ...
    let q: Array2<f32> = Array2::from_shape_fn((seq_len, num_heads * head_dim), |(i, j)| {
        ((i * j) as f32 * 0.01).sin()
    });
    let k: Array2<f32> = Array2::from_shape_fn((seq_len, num_heads * head_dim), |(i, j)| {
        ((i + j) as f32 * 0.01).cos()
    });
    let v: Array2<f32> = Array2::from_shape_fn((seq_len, num_heads * head_dim), |(i, j)| {
...
fn test_flash_attention_3_forward_medium() {
    ...
    let q: Array2<f32> = Array2::from_shape_fn((seq_len, num_heads * head_dim), |(i, j)| {
        ((i as f32 - seq_len as f32 / 2.0) / (head_dim as f32).sqrt())
    });
    let k: Array2<f32> = q.clone(); // 简化：K=Q用于自注意力
    let v: Array2<f32> =
        Array2::from_shape_fn((seq_len, num_heads * head_dim), |(_, j)| j as f32 * 0.1);
...
fn test_block_strategy_validation() {
    ...
    for &(block_size, head_block_size) in &valid_configs {
        assert!(
            block_size % head_block_size == 0 || head_block_size % block_size == 0,
            "分块大小应该合理配对"
        );
...
fn test_performance_scaling_analysis() {
    ...
    for &seq_len in &seq_lengths {
        ...
        let k = q.clone();
        let v: Array2<f32> =
            Array2::from_shape_fn((seq_len, num_heads * head_dim), |(_, j)| j as f32);
