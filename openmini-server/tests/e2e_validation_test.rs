...
impl E2ETestEngine {
    ...
    fn test_tokenization(&self) -> Result<E2ETestResult, String> {
        ...
        Ok(E2ETestResult {
            ...
            error_message: None,
            stats: None,
            output_text: Some(token_counts.iter()
    ...
    fn test_single_generation(&self, prompt: &str) -> Result<E2ETestResult, String> {
        ...
        let generated = tokio_test.block_on(engine.generate_async(prompt, &params))
    ...
    fn test_batch_generation(&self) -> Result<E2ETestResult, String> {
        ...
        for prompt in &self.test_prompts {
            ...
            let output = tokio_test.block_on(engine.generate_async(prompt, &params))
        ...
        let avg_time_per_prompt = total_time.as_millis() as f64 / self.test_prompts.len().max(1) as f64;
    ...
    fn test_greedy_decoding(&self) -> Result<E2ETestResult, String> {
        let prompt = self.test_prompts.first()
            .cloned()
    ...
    fn print_summary(&self) {
        ...
        println!("总测试数: {}", total);
        println!("通过: {} ({:.1}%)", passed, passed as f64 / total.max(1) as f64 * 100.0);
        println!("失败: {} ({:.1}%)", failed, failed as f64 / total.max(1) as f64 * 100.0);
        ...
        // 性能统计
        let successful_stats: Vec<&InferenceStats> = self.results.iter()
            .filter_map(|r| r.stats.as_ref())
            .collect();

        if !successful_stats.is_empty() {
            let avg_speed: f32 = successful_stats.iter()
                .map(|s| s.tokens_per_second)
                .sum::<f32>() / successful_stats.len() as f32;
