...
fn test_full_pipeline_correctness() {
    ...
    for (seq_len, head_dim, num_heads, top_k, use_causal) in test_configs {
        ...
        if num_heads == 1 {
            ...
            assert!(standard_result.is_ok(), "Standard sparse_attention failed for seq_len={}", seq_len);
            let std_output = standard_result.unwrap();
            assert_eq!(std_output.dim(), (seq_len, head_dim));

            for i in 0..seq_len {
                for j in 0..head_dim {
                    assert!(std_output[[i, j]].is_finite(), "Standard output non-finite at [{}, {}] for seq_len={}", i, j, seq_len);
                }
            }

            let optimized_result = sparse_attention_forward_optimized(&q, &k, &v, head_dim, &config, use_causal);
            assert!(optimized_result.is_ok(), "Optimized sparse_attention failed for seq_len={}", seq_len);
            let opt_output = optimized_result.unwrap().0;
            assert_eq!(opt_output.dim(), (seq_len, head_dim));

            let max_diff = std_output.iter().zip(opt_output.iter()).map(|(&s, &o)| (s - o).abs()).fold(0.0f32, |a, b| a.max(b));
            assert!(max_diff < 1e-2, "Standard/Optimized mismatch: max_diff={}", max_diff);

            println!("  [PASS] Single-head seq_len={}, max_diff={:.6}", seq_len, max_diff);
        } else {
            let multihead_result = multihead_sparse_attention(&q, &k, &v, num_heads, head_dim, &config, use_causal);
            assert!(multihead_result.is_ok(), "Multi-head attention failed for heads={}", num_heads);
            let mh_output = multihead_result.unwrap();
            assert_eq!(mh_output.dim(), (seq_len, hidden_size));
            for val in mh_output.iter() { assert!(val.is_finite(), "Multi-head output contains non-finite value"); }
...
fn test_memory_usage_under_load() {
    ...
    let estimate_cases = vec![(1024, 128, 256), (2048, 256, 512), (4096, 128, 128), (8192, 256, 256)];

    for (seq_len, head_dim, top_k) in estimate_cases {
        let estimate = estimate_dsa_memory_usage(seq_len, head_dim, top_k);
        println!("[Estimate] seq_len={}, head_dim={}, k={} => {:.2} MB", seq_len, head_dim, top_k, estimate.qkv_bytes as f64 / (1024.0 * 1024.0));
    ...
    for _ in 0..10 { handles.push(pool.acquire_f32(100000)); }
    drop(handles);
...
fn test_graceful_degradation() {
    ...
    for (seq_len, head_dim) in test_sizes {
        ...
        match lightning_indexer_gpu(&q, &k) {
            Ok(scores) => { println!("  [GPU] Available and successful"); assert_eq!(scores.dim(), (seq_len, seq_len)); }
            Err(e) => { println!("  [GPU] Not available (expected): {}", e); }
        }

        match lightning_indexer_gpu_chunked(&q, &k, None) {
            Ok((scores, stats)) => { println!("  [GPU Chunked] Available: chunks={:?}", stats.chunk_count); assert_eq!(scores.dim(), (seq_len, seq_len)); }
            Err(e) => { println!("  [GPU Chunked] Fallback to CPU: {}", e); }
        }

        let adaptive_result = lightning_indexer_adaptive(&q, &k);
        assert_eq!(adaptive_result.dim(), (seq_len, seq_len), "Adaptive indexer must always succeed");
        println!("  [Adaptive] Success (auto-fallback working)");

        match lightning_indexer_auto(&q, &k) {
            Ok(scores) => { assert_eq!(scores.dim(), (seq_len, seq_len)); println!("  [Auto] Success"); }
            Err(e) => { if seq_len < 1024 { panic!("Auto indexer should succeed for small matrices: {}", e); } else { println!("  [Auto] Failed (acceptable): {}", e); } }
        }

        let scores_matrix = Array2::from_shape_fn((4, seq_len), |(i, j)| ((i * seq_len + j) as f32 * 0.1) % 1.0);
        match top_k_selection_metal(&scores_matrix, 8.min(seq_len / 2)) {
            Ok((indices, _)) => { assert_eq!(indices.len(), 4); println!("  [Top-K Metal] Success"); }
            Err(e) => { println!("  [Top-K Metal] Fallback: {}", e); }
        }

        let cpu_standard = lightning_indexer(&q, &k);
        let cpu_adaptive = lightning_indexer_adaptive(&q, &k);
        let mut max_diff = 0.0f32;
        for i in 0..seq_len { for j in 0..seq_len { let diff = (cpu_standard[[i, j]] - cpu_adaptive[[i, j]]).abs(); if diff > max_diff { max_diff = diff; } } }
