...
use openmini_server::model::inference::dsa::{
    ...
    multihead_sparse_attention, sparse_attention_forward, sparse_attention_forward_optimized,
    top_k_selection_metal, DSAMemoryPool, DSATopKConfig,
};
...
fn test_full_pipeline_correctness() {
    ...
    for (seq_len, head_dim, num_heads, top_k, use_causal) in test_configs {
        ...
        if num_heads == 1 {
            let standard_result =
                sparse_attention_forward(&q, &k, &v, head_dim, &config, use_causal);
            assert!(
            ...
            assert!(
                max_diff < 1e-2,
                "Standard/Optimized mismatch: max_diff={}",
                max_diff
            );

            println!(
                "  [PASS] Single-head seq_len={}, max_diff={:.6}",
                seq_len, max_diff
            );
        } else {
            ...
            println!(
                "  [PASS] Multi-head heads={}, hidden_size={}",
...
fn test_memory_usage_under_load() {
    ...
    for _ in 0..10 {
        handles.push(pool.acquire_f32(100000));
    }
    drop(handles);
