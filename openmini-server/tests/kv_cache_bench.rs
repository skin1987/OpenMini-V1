...
mod performance_tests {
    ...
    use openmini_server::hardware::kv_cache::{
        BatchScheduler, GenerationRequest, PagedKVCache, PrefixCache, PrefixHash,
    };
    ...
    fn test_allocation_performance() {
        ...
        println!(
            "Allocation of 1000 requests (32 tokens each): {:?}",
            elapsed
        );
    ...
    fn test_memory_utilization() {
        ...
        println!(
            "Memory utilization with 50/100 blocks: {:.2}%",
            utilization * 100.0
        );
...
mod stress_tests {
    ...
    use openmini_server::hardware::kv_cache::{BatchScheduler, GenerationRequest, PagedKVCache};
