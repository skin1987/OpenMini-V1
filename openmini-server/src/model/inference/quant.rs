...
impl DynamicQuantizer {
    ...
    pub fn calibrate(&mut self, data: &[f32]) {
        ...
        let range = max - min;
        self.scale = range / match self.quant_type {
    ...
    pub fn dequantize(&self, bits: u64) -> f32 {
        match self.quant_type {
            ...
            QuantType::Int8 => {
                (bits as i8 as f32) * self.scale + self.zero_point
            }
            QuantType::Int4 => {
                ((bits as i8) - 8) as f32 * self.scale + self.zero_point
            }
        }
    }
...
mod tests {
    ...
    fn test_dequantize_q8_k() {
        ...
        // 设置一些非零量化值
        for i in 2..min(258, 18) { // 只设置少量避免过大输出
            data[i] = 0x10; // 值=16
        }
    ...
    fn test_dequantize_iq4_series() {
        ...
        for (i, &val) in result_nl.iter().enumerate() {
            assert!(val.is_finite(), "IQ4_NL result[{}] should be finite, got {}", i, val);
        }
        ...
        for i in 0..n {
            assert!((result_nl[i] - result_xs[i]).abs() < f32::EPSILON,
                "IQ4_XS and IQ4_NL should produce same results at index {}", i);
        }

        // 测试边界情况：数据长度不足时不应 panic
        let short_data = vec![0u8; 10];  // 远小于 130 字节
        let result_short = dequantize_iq4_nl(&short_data, 64);
        assert_eq!(result_short.len(), 64, "Should return zero-filled vector for insufficient data");

        // 测试 n=0 的边界情况
        let result_empty = dequantize_iq4_nl(&data, 0);
        assert!(result_empty.is_empty(), "Empty input should return empty result");
    }
    ...
    fn test_dynamic_quantizer_roundtrip() {
        ...
        for &val in &test_values {
            ...
            assert!((rec - val).abs() < 1e-6, "Fp32 roundtrip failed for {}", val);
        }
        ...
        for &val in &test_values {
            ...
            if val != 0.0 {
                ...
                assert!(relative_error < 0.1 || error < 1.0, "Int8 error too large: {} -> {} (error={})", val, rec, error);
            }
        }
