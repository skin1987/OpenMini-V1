...
impl VulkanInstance {
    ...
    pub fn new() -> Result<Self> {
        ...
        let app_info = vk::ApplicationInfo::default()
            .application_name(unsafe { std::ffi::CStr::from_bytes_with_nul_unchecked(b"OpenMini\0") })
...
impl VulkanQueue {
    ...
    pub fn new(device: std::sync::Arc<VulkanDevice>) -> Result<Self> {
        let queue = unsafe {
            device.device().get_device_queue(device.compute_queue_family_index(), 0)
        };
    ...
    pub fn allocate_command_buffer(&self) -> Result<vk::CommandBuffer> {
        ...
        let command_buffers = unsafe {
            self.device.device().allocate_command_buffers(&command_buffer_allocate_info)?
        };
    ...
    pub fn submit(&self, command_buffer: vk::CommandBuffer, fence: vk::Fence) -> Result<()> {
        ...
        unsafe {
            self.device.device().queue_submit(self.queue, &[submit_info], fence)?;
        }
...
impl VulkanBuffer {
    ...
    pub fn new(device: std::sync::Arc<VulkanDevice>, size: usize) -> Result<Self> {
        ...
        let memory_properties = unsafe {
            device.instance().instance().get_physical_device_memory_properties(device.physical_device())
        };
        ...
        let memory = unsafe { device.device().allocate_memory(&memory_allocate_info, None)? };
...
impl PipelineManager {
    ...
    fn create_pipeline(&mut self, shader_type: ShaderType, binding_count: u32) -> Result<()> {
        ...
        let shader_module = unsafe {
            self.device.device().create_shader_module(&shader_module_create_info, None)?
        };
        ...
        self.descriptor_set_layouts.insert(shader_type, descriptor_set_layout);
    ...
    fn create_descriptor_set_layout(&self, binding_count: u32) -> Result<vk::DescriptorSetLayout> {
        ...
        unsafe {
            Ok(self.device.device().create_descriptor_set_layout(&create_info, None)?)
        }
    }
    ...
    fn create_pipeline_layout(
        ...
        unsafe {
            Ok(self.device.device().create_pipeline_layout(&create_info, None)?)
        }
    }
...
impl DescriptorPool {
    ...
    pub fn allocate_descriptor_set(
        ...
        unsafe {
            Ok(self.device.device().allocate_descriptor_sets(&allocate_info)?[0])
        }
    }
    ...
    pub fn free_descriptor_set(&self, descriptor_set: vk::DescriptorSet) -> Result<()> {
        unsafe {
            self.device.device().free_descriptor_sets(self.pool, &[descriptor_set])?;
        }
        Ok(())
    }
}
...
impl CommandBufferPool {
    /// 创建命令缓冲区池
    pub fn new(device: std::sync::Arc<VulkanDevice>, command_pool: vk::CommandPool, initial_count: u32) -> Result<Self> {
    ...
    pub fn acquire(&self) -> Result<vk::CommandBuffer> {
        ...
        let buffers = unsafe {
            self.device.device().allocate_command_buffers(&allocate_info)?
        };
    ...
    pub fn release(&self, buffer: vk::CommandBuffer) {
        unsafe {
            let _ = self.device.device().reset_command_buffer(
                buffer,
...
impl FencePool {
    ...
    pub fn acquire(&self) -> Result<vk::Fence> {
        ...
        let fence_create_info = vk::FenceCreateInfo::default();
        let fence = unsafe {
            self.device.device().create_fence(&fence_create_info, None)?
        };
...
impl VulkanBackend {
    ...
    pub fn new() -> Result<Self> {
        ...
        let command_buffer_pool = CommandBufferPool::new(
            device.clone(),
            queue.command_pool(),
            16,
        )?;
    ...
    fn execute_kernel(
        ...
        let (pipeline, pipeline_layout, descriptor_set_layout) = {
            let manager = self.pipeline_manager.read()
            ...
            if let Some(result) = manager.get_pipeline_cached(shader_type) {
                ...
                let mut manager = self.pipeline_manager.write()
                    .map_err(|_| anyhow::anyhow!("获取 pipeline_manager 写锁失败"))?;
                manager.get_pipeline(shader_type, binding_count)?
            }
        };
        
        let descriptor_set = if let Some(ds) = self.descriptor_set_pool.acquire(descriptor_set_layout) {
            ds
        } else {
            let mut descriptor_pool = self.descriptor_pool.write()
        ...
        {
            let descriptor_pool = self.descriptor_pool.read()
        ...
        self.descriptor_set_pool.release(descriptor_set, descriptor_set_layout);
    ...
    fn matmul_vulkan(&self, a: &Array2<f32>, b: &Array2<f32>) -> Result<Array2<f32>> {
        ...
        let a_buffer = VulkanBuffer::from_data(self.device.clone(), a.as_slice().ok_or_else(|| anyhow::anyhow!("矩阵 A 不是连续存储"))?)?;
        let b_buffer = VulkanBuffer::from_data(self.device.clone(), b.as_slice().ok_or_else(|| anyhow::anyhow!("矩阵 B 不是连续存储"))?)?;
    ...
    fn softmax_vulkan(&self, input: &Array2<f32>) -> Result<Array2<f32>> {
        ...
        let input_buffer = VulkanBuffer::from_data(self.device.clone(), input.as_slice().ok_or_else(|| anyhow::anyhow!("输入矩阵不是连续存储"))?)?;
        let output_buffer = VulkanBuffer::new(self.device.clone(), rows * cols * std::mem::size_of::<f32>())?;
        
        let dims = [rows as u32, cols as u32];
        let dims_buffer = VulkanBuffer::from_data(self.device.clone(), &dims)?;
        
        let group_count = ((rows as u32 + SOFTMAX_BLOCK_SIZE - 1) / SOFTMAX_BLOCK_SIZE, 1, 1);
    ...
    fn layer_norm_vulkan(
        ...
        let group_count = ((rows as u32 + LAYERNORM_BLOCK_SIZE - 1) / LAYERNORM_BLOCK_SIZE, 1, 1);
        
        self.execute_kernel(
            ShaderType::LayerNorm,
            &[&input_buffer, &output_buffer, &gamma_buffer, &beta_buffer, &dims_buffer, &eps_buffer],
            6,
            group_count,
        )?;
    ...
    pub fn rms_norm(&self, input: &Array2<f32>, gamma: &[f32], eps: f32) -> Result<Array2<f32>> {
        ...
        self.execute_kernel(
            ShaderType::RmsNorm,
            &[&input_buffer, &output_buffer, &gamma_buffer, &dims_buffer, &eps_buffer],
            5,
            group_count,
        )?;
    ...
    fn attention_vulkan(
        ...
        let query_buffer = VulkanBuffer::from_data(self.device.clone(), query.as_slice().ok_or_else(|| anyhow::anyhow!("query 矩阵不是连续存储"))?)?;
        let key_buffer = VulkanBuffer::from_data(self.device.clone(), key.as_slice().ok_or_else(|| anyhow::anyhow!("key 矩阵不是连续存储"))?)?;
        let value_buffer = VulkanBuffer::from_data(self.device.clone(), value.as_slice().ok_or_else(|| anyhow::anyhow!("value 矩阵不是连续存储"))?)?;
        let output_buffer = VulkanBuffer::new(self.device.clone(), seq_len * head_dim * std::mem::size_of::<f32>())?;
        
        let mask_buffer = if let Some(m) = mask {
            VulkanBuffer::from_data(self.device.clone(), m.as_slice().ok_or_else(|| anyhow::anyhow!("mask 矩阵不是连续存储"))?)?
        } else {
        ...
        self.execute_kernel(
            ...
            &[&query_buffer, &key_buffer, &value_buffer, &output_buffer, &mask_buffer, &dims_buffer, &scale_buffer, &has_mask_buffer],
            8,
            group_count,
        )?;
    ...
    pub fn attention_with_kv_cache(
        ...
        let query_buffer = VulkanBuffer::from_data(self.device.clone(), query.as_slice().ok_or_else(|| anyhow::anyhow!("query 矩阵不是连续存储"))?)?;
        let key_buffer = VulkanBuffer::from_data(self.device.clone(), key_cache.as_slice().ok_or_else(|| anyhow::anyhow!("key_cache 矩阵不是连续存储"))?)?;
        let value_buffer = VulkanBuffer::from_data(self.device.clone(), value_cache.as_slice().ok_or_else(|| anyhow::anyhow!("value_cache 矩阵不是连续存储"))?)?;
        let output_buffer = VulkanBuffer::new(self.device.clone(), head_dim * std::mem::size_of::<f32>())?;
        ...
        self.execute_kernel(
            ...
            &[&query_buffer, &key_buffer, &value_buffer, &output_buffer, &cache_info_buffer, &scale_buffer],
            6,
            group_count,
        )?;
...
mod tests {
    ...
    fn test_attention_kv_cache() {
        ...
        let result = backend.attention_with_kv_cache(&query, &key_cache, &value_cache, kv_len).unwrap();
    ...
    fn test_matmul_dimension_mismatch() {
        ...
        let b = Array2::from_shape_vec((4, 2), vec![
            1.0; 8
        ]).unwrap(); // K 不匹配
...
mod benches {
    ...
    fn benchmark_matmul(backend: &VulkanBackend, m: usize, k: usize, n: usize, iterations: usize) {
        ...
        println!(
            "Matmul {}x{}x{}: {:?} ({:.2} GFLOPS)",
            m, k, n, elapsed / iterations as u32, gflops
        );
    }
    ...
    fn benchmark_softmax(backend: &VulkanBackend, rows: usize, cols: usize, iterations: usize) {
        ...
        println!(
            "Softmax {}x{}: {:?}",
            rows, cols, elapsed / iterations as u32
        );
    }
    
    fn benchmark_attention(backend: &VulkanBackend, seq_len: usize, head_dim: usize, iterations: usize) {
        let query = Array2::from_shape_fn((seq_len, head_dim), |(i, j)| ((i + j) as f32));
        ...
        println!(
            ...
            seq_len, head_dim, elapsed / iterations as u32
        );
    }
