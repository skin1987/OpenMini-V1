...
use crate::hardware::{
    ...
    TaskType, CoreSelectionStrategy,
};
...
impl ThreadPool {
    ...
    pub fn with_config(config: PoolConfig) -> ThreadPool {
        ...
        let core_assignments = affinity.select_cores(config.core_strategy, Some(config.num_threads));
...
mod tests {
    ...
    fn test_thread_pool_execute_multiple_tasks() {
        ...
        // 验证结果包含所有预期的值(顺序可能不同)
        for i in 0..10 {
            assert!(final_results.contains(&(i * i)), 
                "Should contain result for input {}", i);
        }
    }
    ...
    fn test_thread_pool_shutdown_and_restart() {
        ...
        // 验证任务已完成
        assert_eq!(counter.load(Ordering::SeqCst), 5, "All tasks should complete before shutdown");
    }
    ...
    fn test_thread_pool_panic_recovery() {
        ...
        // 正常任务应该成功执行
        assert_eq!(counter.load(Ordering::SeqCst), 1, "Normal task should execute after panic");
    }
    ...
    fn test_thread_pool_high_priority_tasks() {
        ...
        assert_eq!(total, 33, "All tasks (normal + high priority) should complete: got {}", total);
    }
    ...
    fn test_thread_pool_task_ordering_guarantees() {
        ...
        for i in 0..5 {
            ...
            assert!(second_pos > first_pos,
                "Task {}'s second operation should come after first", i);
        }
    }
}
