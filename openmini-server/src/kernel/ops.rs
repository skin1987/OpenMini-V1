...
pub fn add(a: &ArrayView1<f32>, b: &ArrayView1<f32>) -> Result<Array1<f32>> {
    if a.len() != b.len() {
        return Err(anyhow::anyhow!("向量长度不匹配: {} vs {}", a.len(), b.len()));
    }
...
pub fn gelu(x: &ArrayView1<f32>) -> Array1<f32> {
    x.mapv(|v| {
        let cdf = 0.5 * (1.0 + (2.0 / std::f32::consts::PI).sqrt() * (v + 0.044715 * v.powi(3)).tanh());
        v * cdf
    })
}
...
mod tests {
    ...
    fn test_matmul_dimension_mismatch() {
        ...
        assert!(err_msg.contains("维度不匹配"), "错误信息应包含维度不匹配提示");
    }
    ...
    fn test_layer_norm_normal() {
        ...
        assert!((var - 1.0).abs() < 0.01, "LayerNorm后方差应接近1，实际: {}", var);
    }
    ...
    fn test_rms_norm_normal() {
        ...
        assert!((rms - 1.0).abs() < 0.01, "RMSNorm后RMS应接近1，实际: {}", rms);
    }
