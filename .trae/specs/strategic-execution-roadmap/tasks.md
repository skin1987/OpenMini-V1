...
- [ ] **P0-1: NSA (Native Sparse Attention) 集成**
  - [ ] P0-1.1: 创建 `nsa.rs` 文件，定义 `NativeSparseAttention` 结构体
  - [ ] P0-1.2: 实现 TokenCompressor 压缩策略 (保留全局信息)
  - [ ] P0-1.3: 实现 TopKSelector 选择策略 (保留关键细节)
  - [ ] P0-1.4: 实现 SlidingWindowAttention 滑动窗口策略
  - [ ] P0-1.5: 实现 NSAFusionLayer 三路输出融合层
  - [ ] P0-1.6: 实现与 MLA 的协同选择逻辑 (短→FA3, 中→MLA, 长→NSA)
  - [ ] P0-1.7: 编写 NSA 单元测试 (正确性验证)
  - [ ] P0-1.8: 性能基准测试 (32K序列 vs DSA对比)

- [ ] **P0-2: Kascade (锚点层复用稀疏注意力)**
  - [ ] P0-2.1: 创建 `kascade.rs`，定义 `KascadeSparseAttention` 结构
  - [ ] P0-2.2: 实现锚点层选择算法 (DP动态规划最大化跨层相似度)
  - [ ] P0-2.3: 实现精确 Top-K 索引计算与缓存
  - [ ] P0-2.4: 实现三种复用策略 (Direct/Weighted/Adaptive)
  - [ ] P0-2.5: 集成到现有推理引擎 `inference.rs`
  - [ ] P0-2.6: 单元测试 + 集成测试

- [ ] **P0-3: Native Top-K Sparse Attention (美团)**
  - [ ] P0-3.1: 创建 `native_top_k.rs`，实现 GPU kernel 版本
  - [ ] P0-3.2: 实现启发式 top-k 选择 (无需SFT的快速模式)
  - [ ] P0-3.3: 升级 Lightning Indexer 为 Native Top-K
  - [ ] P0-3.4: 可选 SFT 微调接口 (提升精度至99.5%+)
  - [ ] P0-3.5: 替换 dsa.rs 中的 Lightning Indexer 调用

- [ ] **P0-4: AMLA (Addition代替Multiplication in FA)**
  - [ ] P0-4.1: 在 `flash_attention_3.rs` 中实现 `amla_rescale()` 函数
  - [ ] P0-4.2: FP8量化指数差 → 整数加法转换逻辑
  - [ ] P0-4.3: SIMD优化整数加法版本
  - [ ] P0-4.4: AMLA模式开关 (可与标准FA3切换)
  - [ ] P0-4.5: 性能测试 (吞吐量提升15-25%验证)

- [ ] **P0-5: CUDA Kernel Phase 1 (GPU加速基础)**
  - [ ] P0-5.1: 创建 `kernel/cuda/mod.rs`，CUDA上下文初始化与管理
  - [ ] P0-5.2: 实现 `kernel/cuda/memory.rs` (cudaMallocAsync分配器)
  - [ ] P0-5.3: 实现 `kernel/cuda/matmul.rs` (cuBLAS GEMM wrapper)
  - [ ] P0-5.4: 实现 `kernel/cuda/flash_attention.rs` (FA3 CUDA kernel)
  - [ ] P0-5.5: 实现 `kernel/cuda/quant.rs` (18种格式GPU反量化)
  - [ ] P0-5.6: 硬件检测与自动后端选择 (CPU/GPU/Metal)
  - [ ] P0-5.7: RTX 4090/H100 性能达标测试

- [ ] **P0-6: 模型架构生态扩展 (至15种)**
  - [ ] P0-6.1: 在 `gguf.rs` 添加 Mistral 架构支持 (`mistral` 前缀)
  - [ ] P0-6.2: 添加 Mixtral MoE 架构支持 (`mixtral` 前缀, 8专家)
  - [ ] P0-6.3: 添加 Yi 系列架构支持 (`yi` 前缀)
  - [ ] P0-6.4: **修复** Phi系列参数读取bug (使用正确前缀)
  - [ ] P0-6.5: 添加 ChatGLM3 架构支持 (`chatglm`, PrefixLM)
  - [ ] P0-6.6: 添加 Baichuan 2 架构支持 (`baichuan`)
  - [ ] P0-6.7: 每种架构编写加载测试 (GGUF文件解析验证)

- [ ] **P0-7: 公开 Benchmark 与 CI性能回归**
  - [ ] P0-7.1: 创建 `benchmark/` 目录结构
  - [ ] P0-7.2: 实现 `BenchmarkConfig` / `ModelBenchmark` / `Scenario` 结构
  - [ ] P0-7.3: 实现核心指标采集 (TTFT/TPOT/TBTL/Throughput/Memory)
  - [ ] P0-7.4: 实现结果导出 (JSON/CSV/Prometheus格式)
  - [ ] P0-7.5: CI集成 (GitHub Actions workflow, PR触发)
  - [ ] P0-7.6: 性能基线建立 (首次运行记录为baseline)
