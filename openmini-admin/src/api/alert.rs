...
use crate::db::models::{AlertRecord, AlertRule};
...
pub async fn list_records(
    ...
    let rows =
        sqlx::query("SELECT ar.* FROM alert_records ar ORDER BY ar.fired_at DESC LIMIT ? OFFSET ?")
            .bind(size)
            .bind((page - 1) * size)
            .fetch_all(&*state.pool)
            .await?;
    ...
    Ok(Json(
        serde_json::json!({ "items": records, "total": count.0 as u64, "page": page as u64, "page_size": size as u64 }),
    ))
}
...
pub async fn get_summary(
    ...
    let firing: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM alert_records WHERE status = 0")
        .fetch_one(&*state.pool)
        .await?;
    let acked: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM alert_records WHERE status = 1")
        .fetch_one(&*state.pool)
        .await?;
    let resolved: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM alert_records WHERE status = 2")
        .fetch_one(&*state.pool)
        .await?;
