...
pub fn create_admin_routes(state: &AppState) -> Router<AppState> {
    ...
    let protected = Router::new()
        ...
        .route("/admin/model/:id", axum::routing::get(model::get_model_detail).put(model::update_model_config))
        ...
        // ==================== 会话监控 ====================
        .route("/admin/sessions", axum::routing::get(session::list_active_sessions))
        .route("/admin/sessions/:session_id", axum::routing::get(session::get_session_detail))
        .route("/admin/sessions/stats", axum::routing::get(session::get_session_stats))
        .route("/admin/sessions/:session_id/terminate", axum::routing::post(session::terminate_session))
        .route("/admin/sessions/cleanup", axum::routing::post(session::cleanup_expired_sessions))

        // ==================== 指标仪表盘 ====================
        .route("/admin/metrics/system", axum::routing::get(metrics::system_metrics))
        .route("/admin/metrics/inference", axum::routing::get(metrics::inference_metrics))
        .route("/admin/metrics/history", axum::routing::get(metrics::metrics_history))
        .route("/admin/metrics/dashboard", axum::routing::get(metrics::dashboard_overview))
        .route("/admin/metrics/alerts/thresholds", axum::routing::get(metrics::alert_thresholds))
        ...
        .route("/admin/config/restart", axum::routing::post(config::apply_config_restart))
        .route("/admin/config/history", axum::routing::get(config::get_history))
        .route("/admin/config/validate", axum::routing::post(config::validate_config))
        .route("/admin/config/export", axum::routing::get(config::export_config))
